...
import { useToast } from './ToastSystem.tsx';
...
const Login: React.FC<LoginProps> = ({ onLoginSuccess }) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.user.name.includes("Offline Mode")) {
          toast.warning("Backend Unreachable. Logged in via Safe Mode.", "System Notice");
      } else {
          toast.success(`Welcome back, ${response.user.name}`, "Secure Login");
      }
      
      onLoginSuccess(response);
    } catch (error: any) {
      if (error.message === "BACKEND_OFFLINE") {
          toast.error("Database Connection Timed Out. Please check your VPS status.", "Network Failure");
      } else {
          toast.error(error.message || "Invalid access key or identity", "Access Denied");
      }
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="min-h-screen flex items-center justify-center p-6 relative overflow-hidden bg-dark-bg selection:bg-neon-teal selection:text-black">
      ...
      <div className="max-w-md w-full animate-fade-in relative z-10">
        ...
        <div className="flex flex-col items-center mb-10">
          ...
          <p className="text-slate-500 mt-2 font-medium tracking-wide">SECURE ACCESS PORTAL</p>
        </div>
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2">
                <UserIcon className="w-3 h-3 text-neon-teal" /> Identity
              </label>
              <input 
                type="text" 
                value={email}
            ...
            <div className="flex items-center justify-between pt-2">
               <label className="flex items-center gap-2 cursor-pointer group">
                  ...
                  <span className="text-xs text-slate-400">Trusted Terminal</span>
               </label>
               <div className="flex items-center gap-1 text-[10px] text-slate-600 italic">
                  <WifiOff className="w-3 h-3" /> Auto-Fallback Enabled
               </div>
            </div>
