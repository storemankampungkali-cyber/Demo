...
import { LayoutDashboard, Package, Bot, Settings, LogOut, Hexagon, ShoppingCart, History, Ban } from 'lucide-react';
  ...
  return (
    <div className="w-20 lg:w-64 h-screen fixed left-0 top-0 bg-dark-bg border-r border-dark-border flex flex-col z-50 transition-all duration-300">
        ...
        {menuItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`
                group flex items-center px-3 lg:px-4 py-3.5 rounded-xl transition-all duration-300 relative overflow-hidden
                ${isActive 
            ...
            >
              ...
              <Icon className={`w-6 h-6 ${isActive ? 'drop-shadow-[0_0_5px_rgba(0,242,234,0.5)]' : ''}`} />
              <span className={`hidden lg:block ml-4 font-medium ${isActive ? 'translate-x-1' : ''} transition-transform`}>
                {item.label}
              </span>
            </button>
          );
      ...
      <div className="p-4 border-t border-dark-border/50">
        <button className="flex items-center w-full px-4 py-3 text-slate-400 hover:text-red-400 transition-colors rounded-xl hover:bg-red-500/10 mb-2">
          <LogOut className="w-5 h-5" />
          <span className="hidden lg:block ml-3 font-medium">Log Out</span>
        </button>
        <div className="hidden lg:block text-center">
            <span className="text-[10px] text-slate-600 uppercase tracking-widest font-semibold">v1.3 Reject</span>
        </div>
      </div>
    </div>
  );
